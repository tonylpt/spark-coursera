...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row.fromSeq(line.head :: line.tail.map(token => token.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val workingCols = List("t05", "t1805")
    val otherCols = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val groups = columnNames.groupBy {
      case col if primaryNeedsCols.exists(col.startsWith) => "primary"
      case col if workingCols.exists(col.startsWith) => "working"
      case col if otherCols.exists(col.startsWith) => "other"
      case _ => "ignored"
    }

    val colGroups = groups.mapValues(l => l.map(col))

    (
      colGroups.getOrElse("primary", List()),
      colGroups.getOrElse("working", List()),
      colGroups.getOrElse("other", List())
    )
  }
    ...
    summed
      ...
      .map { case ((working, sex, age), primaryNeeds, work, other) =>
        TimeUsageRow(working, sex, age,
          math.round(primaryNeeds * 10) / 10.0,
          math.round(work * 10) / 10.0,
          math.round(other * 10) / 10.0
        )
      }
      .orderBy($"working", $"sex", $"age")
  }
